...
LF.lfsocialhub.prototype._prepData = function() {
	...
	define('lfsocialhub',this);
	require([
	         'streamhub-sdk/content/views/content-list-view',
	         'streamhub-sdk/content/views/rss-content-view', 
            'streamhub-sdk/content/views/rss-content-list-view',
	        'lfsocialhub'],
	    function(ListView, RssView, RssListView, lfSocialHub) {
			var Collection = Livefyre.require('streamhub-sdk/collection');
			
			for (var i = 0; i < lfSocialHub.opts.collections.length; ++i) {
				var collection = lfSocialHub.opts.collections[i];

				lfSocialHub.links.push(collection.name); // for the headers
				
				lfSocialHub.collections[collection.name + "Collection"] = new Collection({
					network: collection.network,
					siteId: collection.siteId,
					articleId: collection.articleId
				});
						
				if (!lfSocialHub.isHandheld) { // don't do lfSocialHub for handheld since we're going to do a 1-column view
					lfSocialHub.views[collection.name + "View"] = new ListView({
						initial: 50,
						el: $('#' + collection.name + "Feed")
					});
					
					lfSocialHub.collections[collection.name + "Collection"].pipe(lfSocialHub.views[collection.name + "View"]);
				}
			}
			
			lfSocialHub._setEvents();
	});
